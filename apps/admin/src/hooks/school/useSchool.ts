  ...
  return useQuery(
    ...
    async () => {
      const { data } = await api.get(
        `${routes.SCHOOLS.GET} ${perPage ? `?perPage=${perPage}` : ''}${page ? `&page=${page}` : ''}${name ? `&name=${name}` : ''}${minted ? `&minted=${minted}` : ''}${uploadId ? `&uploadId=${uploadId}` : ``}${country ? `&country=${country}` : ``}${connectivity ? `&connectivityStatus=${connectivity}` : ``}`
      );
      return data;
    },
    {
