  ...
  return (
    <DashboardLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
        <span style={{ fontSize: '1.5em', fontWeight: '600' }}>sss</span>
        <div style={{ display: 'flex', gap: '15px' }}>
          {isMinting ? (
            <Button variant="contained" style={{ background: '#474747' }} onClick={mintSchool}>
              Minting ({selectedValues.length}) NFT's
            </Button>
          ) : (
            <Button variant="contained" style={{ background: '#474747' }} onClick={mintSchool}>
              Mint {selectedValues.length > 0 && `(${selectedValues.length})`}{' '}
            </Button>
          )}
          <Button variant="contained" style={{ background: '#404040' }} onClick={uploadSchool}>
            Import School
          </Button>
        </div>
      </div>
      ...
      {uploadId && (
        <span style={{ color: '#008000', fontSize: '0.85em' }}>
          Recently imported school,{' '}
          <span
            onClick={() => push(`/school/un-minted`)}
            style={{ color: '#795CB2', cursor: 'pointer' }}
          >
            List all
          </span>
        </span>
