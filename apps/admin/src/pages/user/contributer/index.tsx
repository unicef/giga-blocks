...
const UserList = () => {
  ...
  const { data, isFetching, refetch } = useUserGet(
    page,
    rowsPerPage,
    'CONTRIBUTOR',
    debouncedName,
  ...
  return (
    <DashboardLayout>
      ...
      <TextField
        ...
        onChange={(e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => handleSearchChange(e)}
      />
      <Card sx={{ marginTop: 2 }}>
        <Divider />
        <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
          <Scrollbar>
            <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                {tableData && tableData?.map((row: any) => <UserListRow key={row.id} row={row} />)}
