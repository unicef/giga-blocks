...
async function fetchNftDataFromSubgraph(): Promise<NftData[] | null> {
    ...
    try {
        ...
        const response: AxiosResponse<SubgraphGraphQLResponse> = await axiosInstance.post(SUBGRAPH_URL!, { query });
...
async function callApiWithSchoolId(schoolId: string): Promise<any | null> {
    ...
    try {
        console.log(`[API Call] Sending request for schoolId: ${schoolId} to: ${API_ENDPOINT}`);
        const response: AxiosResponse<any> = await axiosInstance.post(API_ENDPOINT!, payload); // Use ! for non-null assertion
...
async function main(): Promise<void> {
    console.log(`Script started at ${new Date().toLocaleString('en-NP', { timeZone: 'Asia/Kathmandu' })}`);
    console.log("--------------------------------------------------");
    ...
    console.log(`Script finished at ${new Date().toLocaleString('en-NP', { timeZone: 'Asia/Kathmandu' })}`);
}
